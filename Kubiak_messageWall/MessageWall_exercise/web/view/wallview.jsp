...
<%
    UserAccess connect = (UserAccess)session.getAttribute("useraccess");
    Message lastMessage = connect.getLast();
...
<body>
    ...
    <h3>user: <em><%=connect.getUser() %></em>
        <a href=logout.do>[Close session]</a></h3>

    <h2> <%=connect.getNumber()%> Messages shown:</h2>
    ...
    <table width="50%" border="1" bordercolordark="#000000" bordercolorlight="#FFFFFF" cellpadding="3" cellspacing="0">
        ...
        <%
            int counter = 0;
            for (Message message : connect.getAllMessages()) {
        %>
